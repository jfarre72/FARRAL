...
export default function MaterialesPage() {
  ...
  // por separado. Además, un acopio puede recibir "sumas" (filas hijas con
  // acopio_padre_id, generadas al sumar plata desde Caja): se consolidan dentro
  // del acopio padre (no son acopios separados).
  const anticiposReales = (cuentaId) => anticiposDe(cuentaId).filter(a => !a.es_devolucion && !a.acopio_padre_id);
  const devolucionesDe = (cuentaId) => anticiposDe(cuentaId).filter(a => a.es_devolucion);
  // Filas hijas (sumas de plata) de un acopio padre.
  const hijosDe = (padreId) => anticipos.filter(a => a.acopio_padre_id === padreId);
  // Monto consolidado de un acopio: su monto + el de sus sumas.
  const montoAcopio = (a) => Number(a.monto || 0) + hijosDe(a.id).reduce((s, h) => s + Number(h.monto || 0), 0);
  ...
  const tcDe = (c) => {
    ...
    // Incluye las sumas (filas hijas): heredan el TC del acopio padre, así que
    // su ARS también pondera el TC de la cuenta.
    const reales = anticiposDe(c.id).filter(a => !a.es_devolucion && Number(a.tipo_cambio || 0) > 0 && Number(a.monto || 0) > 0);
                ...
                <Section title="Anticipos / acopios" accent="#1E8E3E"
                  ...
                  {ant.length > 0 && (
                    <Box sx={{ mt: 1.5 }}>
                      ...
                      <Stack>
                        {ant.map((a) => {
                          const nSumas = hijosDe(a.id).length;
                          const mAcopio = montoAcopio(a);
                          return (
                          <Stack key={a.id} direction="row" alignItems="center" spacing={1}
                            ...
                            <Stack sx={{ flex: 1 }}>
                              <Typography variant="body2" sx={{ fontWeight: 600, color: "#1E8E3E" }}>{fmtMoney(mAcopio, c.moneda)}</Typography>
                              {nSumas > 0 && (
                                <Typography variant="caption" color="text.secondary">
                                  incluye {nSumas} suma{nSumas > 1 ? "s" : ""} · base {fmtMoney(a.monto, c.moneda)}
                                </Typography>
                              )}
                            </Stack>
                            ...
                            {esARS(c) && <Typography variant="body2" sx={{ width: 110, textAlign: "right", color: "text.secondary" }}>{Number(a.tipo_cambio || 0) > 0 ? fmtMoney(mAcopio / Number(a.tipo_cambio), "USD") : "—"}</Typography>}
                            {(() => {
                        ...
                        <TextField select label="Lista de precios (acopio)" fullWidth size="small"
                          ...
                          {listasCta.map(a => (
                            <MenuItem key={a.id} value={a.id}>
                              {(a.fecha ? fmtDate(a.fecha) : "s/f")} · {fmtMoney(montoAcopio(a), c.moneda)}
            ...
            <>
              <DialogTitle>
                ...
                <Typography variant="caption" color="text.secondary" sx={{ display: "block" }}>
                  {a.fecha ? fmtDate(a.fecha) : "sin fecha"} · {fmtMoney(montoAcopio(a), c.moneda)} — congelada dentro de este acopio
                </Typography>
              </DialogTitle>
              <DialogContent dividers>
